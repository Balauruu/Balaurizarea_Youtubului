...
def init_project(
    ...
    5. Append the selected topic to ``context/channel/past_topics.md``
...
def load_project_inputs(root: Path, topic_number: int) -> dict:
    ...
    Reads the selected topic brief from ``context/topics/topic_briefs.md``
    and extracts the Title Patterns section from
    ``context/competitors/analysis.md`` (if present).
