...
def test_load_source_files_reads_both_passes(tmp_path: Path) -> None:
    """Given a dir with 2 src_*.json and 1 pass2_*.json in sources/, returns (pass1=[2], pass2=[1])."""
    sources_dir = tmp_path / "sources"
    sources_dir.mkdir()
    _write_src(sources_dir, "src_001.json", _make_src(1, url="https://a.com"))
    _write_src(sources_dir, "src_002.json", _make_src(2, url="https://b.com"))
    _write_src(sources_dir, "pass2_001.json", _make_src(1, url="https://c.com"))
...
def test_load_source_files_no_pass2(tmp_path: Path) -> None:
    """Given only src_*.json files in sources/, returns (pass1=[items], pass2=[]) without error."""
    sources_dir = tmp_path / "sources"
    sources_dir.mkdir()
    _write_src(sources_dir, "src_001.json", _make_src(1))
    _write_src(sources_dir, "src_002.json", _make_src(2))
...
def test_load_source_files_empty_dir(tmp_path: Path) -> None:
    """Given directory with no sources/ subfolder, returns ([], [])."""
    pass1, pass2 = load_source_files(tmp_path)
...
def test_write_synthesis_input_creates_file(tmp_path: Path) -> None:
    """Writes synthesis_input.md to output_dir/sources/ and returns the path."""
    content = "# Synthesis Input\n\nTest content here.\n"

    result_path = write_synthesis_input(tmp_path, content)

    assert result_path == tmp_path / "sources" / "synthesis_input.md"
