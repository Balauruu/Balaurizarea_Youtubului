...
def test_domain_field_in_src_json(tmp_path: Path) -> None:
    ...
    import json
    src_file = output_dir / "sources" / "src_001.json"
    assert src_file.exists(), "src_001.json not created in sources/"
...
def test_ddg_page_not_saved_as_src(tmp_path: Path) -> None:
    ...
    import json
    for f in (output_dir / "sources").glob("src_*.json"):
...
def test_cmd_deepen_reads_recommended_only(tmp_path: Path) -> None:
    ...
    # Write src files so budget guard passes (now in sources/ subfolder)
    sources_dir = output_dir / "sources"
    sources_dir.mkdir()
    (sources_dir / "src_001.json").write_text(_json.dumps({"url": "https://en.wikipedia.org/wiki/X"}), encoding="utf-8")
    (sources_dir / "src_002.json").write_text(_json.dumps({"url": "https://bbc.com/article"}), encoding="utf-8")
...
def test_cmd_deepen_writes_pass2_json(tmp_path: Path) -> None:
    ...
    pass2_file = sources_dir / "pass2_001.json"
    assert pass2_file.exists(), "pass2_001.json not written in sources/"
...
def test_cmd_deepen_budget_guard(tmp_path: Path, capsys) -> None:
    ...
    # Create 14 existing src_*.json files in sources/
    sources_dir = output_dir / "sources"
    sources_dir.mkdir()
    for i in range(1, 15):
        (sources_dir / f"src_{i:03d}.json").write_text(
...
def test_cmd_deepen_dedup_pass1_urls(tmp_path: Path) -> None:
    ...
    sources_dir = output_dir / "sources"
    sources_dir.mkdir()
    (sources_dir / "src_001.json").write_text(
...
def test_cmd_deepen_clean_exit_no_urls(tmp_path: Path, capsys) -> None:
    ...
    sources_dir = output_dir / "sources"
    sources_dir.mkdir()
    (sources_dir / "src_001.json").write_text(_json.dumps({"url": "https://bbc.com/article"}), encoding="utf-8")
    _write_manifest(output_dir, sources)
...
def test_cmd_deepen_cleans_old_pass2_files(tmp_path: Path) -> None:
    ...
    # Create a stale pass2 file from a previous run (in sources/)
    sources_dir = output_dir / "sources"
    sources_dir.mkdir()
    stale_file = sources_dir / "pass2_001.json"
